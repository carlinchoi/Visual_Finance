...
public class AuctionServiceTest {
    ...
    public static void setup() {
        ...
        YahooAPI.API_BASE_URL = "http://localhost:3001/auctions/";
        sut = new YahooAPI();

        if (!pingServer(YahooAPI.API_BASE_URL, 10, 500)) { // time-out after ~5 seconds
            throw new RuntimeException("Unable to connect to server: " + YahooAPI.API_BASE_URL + System.lineSeparator() + "Make sure you've run `npm install` in the `server` folder and can run json-server.");
        }
    }
